...
template <socket_type type> class iostream {
  ...
  public:
    ...
    bool setDestination(url u) {
        ...
        if (status != 0) {
            debug::thread_safe::cerr << "DNS Lookup failed for " << u.domain << ": " << gai_strerror(status)
                                     << std::endl;
            return false;
        } else
            debug::thread_safe::cout << "DNS Lookup worked! " << u.domain << '\n' << std::flush;
    ...
    iostream(int32_t sock_fd, const sockaddr_in &peer_addr, bool owns_fd = true) : sock_fd(sock_fd), owns_fd(owns_fd) {
        dest_addr = peer_addr;
        char ip_str[INET_ADDRSTRLEN];
        inet_ntop(AF_INET, &peer_addr.sin_addr, ip_str, INET_ADDRSTRLEN);
        dest = std::string(ip_str) + ":" + std::to_string(ntohs(peer_addr.sin_port));
    }
    iostream(url u) {
    ...
    iostream() {}
    ~iostream() {
        if (owns_fd && sock_fd >= 0) {
            close(sock_fd);
        }
    }
    size_t bytes_read() { return last_bytes_read; }
    void send(const char *buffer, size_t length, int flags = 0) {
        size_t total_sent;
        while (length != 0) {
            size_t sent;
            if constexpr (type == socket_type::TCP) {
                sent = ::send(sock_fd, buffer, length, flags);
            } else if constexpr (type == socket_type::UDP) {
                sent = ::sendto(sock_fd, buffer, length, flags, (struct sockaddr *)&dest_addr, sizeof(dest_addr));
            }
            if (sent <= 0) {
                debug::thread_safe::cerr << "send error" << std::endl;
                break;
            } else {
                length -= sent;
                total_sent += sent;
        ...
        debug::thread_safe::cout << dest << " <- (sent " << std::to_string(total_sent) << " bytes) "
                                 << to_hex(std::string(buffer, length)) << std::endl;
    }
    ssize_t recv(char *buffer, size_t total, int flags = 0) const {
        ...
        if constexpr (type == socket_type::UDP) {
            ...
            received = ::recv(sock_fd, buffer, total, flags);
        debug::thread_safe::cout << dest << " -> (recv " << std::to_string(received) << " bytes) "
                                 << to_hex(std::string(buffer, total)) << std::endl;
        return received;
    }
    ssize_t recv_all(char *buffer, size_t total, int flags = 0) const {
        ...
        debug::thread_safe::cout << dest << " -> (recv_all " << std::to_string(total) << " bytes) "
                                 << to_hex(std::string(buffer, total)) << std::endl;
        return total;
    }
    void attach(int32_t fd) { sock_fd = fd; }
    int getTimeout() { return timeout; }
    template <typename T>
        requires std::invocable<T, iostream &> || std::is_convertible_v<T, std::string_view> ||
                 std::is_same_v<std::decay_t<T>, std::vector<char>> || std::is_integral_v<std::decay_t<T>> ||
    ...
    template <typename T>
        requires std::is_same_v<std::decay_t<T>, std::string> || std::is_same_v<std::decay_t<T>, std::vector<char>> ||
